...
def test_store_and_load_tokenizer(tmpdir):
    corpus = [
     'This is the first document.',
     'This document is the second document.',
     'And this is the third one.',
    ...
    dump_dir = tmpdir.mkdir("test-persistence").realpath()
    dump_tokenizer_to_dir(tokenizer, dump_dir)
    tokenizer_loaded = load_tokenizer_from_dir(dump_dir)
    assert np.array_equal(tokenizer.index_docs, tokenizer_loaded.index_docs)
    assert tokenizer.texts_to_sequences(["This is one"]) == \
        tokenizer_loaded.texts_to_sequences(["This is one"])
...
def test_diff_version(caplog):
    ...
    if sklearn.__version__ == "0.21.3":
        ...
        load_vectorizer_from_dir(vectorizer_dir)
